...
| Layer | What |
|---|---|
| **AI Agent** | Python + Flask backend running locally |
| **AI Provider** | [NVIDIA API](https://build.nvidia.com) — free serverless models |
...
## Running Your Own Ava

Ava runs on your local machine. The Python backend handles all AI logic and conversation storage; the userscript connects it to OmegleApp in your browser.

### Prerequisites

- **Python 3.10+**
- **A browser extension** — Tampermonkey or Violentmonkey
- **A free NVIDIA API key** from [build.nvidia.com](https://build.nvidia.com)
- **A GitHub personal access token** with `repo` scope (for saving conversations)

### Quick Start

```bash
# 1. Clone the repo
git clone https://github.com/YOUR_USERNAME/ava-convos
cd ava-convos

# 2. Set up environment
cp .env.example .env
# Edit .env — add your NVIDIA_API_KEY and GITHUB_TOKEN
...
# Open Tampermonkey → Create a new script → paste userscript.js content → Save
```

Once the server is running and the userscript is active, visit [OmegleApp.me](https://omegleapp.me) — Ava will take over automatically. Conversations are saved to the GitHub repo and published here on the archive site.

### Stopping

Press **Ctrl+Q** in the terminal to stop the server.
